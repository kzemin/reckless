...
//  Reckless
//
//  Created by Facundo Kzemin on 19/07/2024.
//
...
@MainActor
final class NavigationMapViewController: UIViewController {
    ...
    init(navigation: Navigation) {
        ...
        navigationMapView.viewportPadding = UIEdgeInsets(top: 20, left: 20, bottom: 20, right: 20)
    ...
    private func requestRoute(to mapPoint: MapPoint) async {
        do {
            ...
            presentAlert(message: "Request failes: \(error.localizedDescription)")
        }
    }
}

extension NavigationMapViewController: NavigationMapViewDelegate {
    func navigationMapView(_ navigationMapView: NavigationMapView, didSelectRouteIndexes routeIndexes: [Int]) {
        guard let index = routeIndexes.first else { return }
        Task {
            await navigation.selectAlternativeRoute(at: index - 1)
        }
    }
